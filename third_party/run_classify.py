...
def SGS_backward(args, loss, model, optimizer, oracle_datasets):
  ...
  def get_random_batch():
    ...
    if args.model_type != "distilbert":
      ...
      inputs["token_type_ids"] = oracle_batch[2] if args.model_type in ["bert"] else None
...
def train(args, train_dataset, model, tokenizer, oracle_datasets, lang2id=None):
  """Train the model."""
...
def load_and_cache_examples(args, task, tokenizer, split='train', language='en', lang2id=None, evaluate=False, few_shot=-1):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    logger.info("all languages = {}".format(language))
    features = []

    for lg in langs:
      split_lg = "dev" if split == "train" and lg != "en" else split
      logger.info("Creating features from dataset file at %s", args.data_dir)
      label_list = processor.get_labels()
      if split_lg == 'train':
        examples = processor.get_train_examples(args.data_dir, lg)
      elif split_lg == 'translate-train':
        examples = processor.get_translate_train_examples(args.data_dir, lg)
      elif split_lg == 'translate-test':
        examples = processor.get_translate_test_examples(args.data_dir, lg)
      elif split_lg == 'dev':
        examples = processor.get_dev_examples(args.data_dir, lg)
      elif split_lg == 'pseudo_test':
        examples = processor.get_pseudo_test_examples(args.data_dir, lg)
      else:
        examples = processor.get_test_examples(args.data_dir, lg)
      ...
      count_tgt = {0 : few_shot, 1:few_shot, 2:few_shot}
      features_sampled = []
      if few_shot > 0 and lg != "en":
        random.seed(args.seed)
        while count != count_tgt:
          candidate = random.choice(features_lg)
          if candidate not in features_sampled and count[candidate.label] < few_shot:
            features_sampled.append(candidate)
            count[candidate.label] += 1
        
        logger.info("Original no. of examples = {}".format(len(features_lg)))
        logger.info('Using few-shot learning on {} examples'.format(len(features_sampled)))
        logger.info("few-shot samples are {}".format(str([f.input_ids for f in features_sampled])))
        features.extend(features_sampled)
      else:
...
def main():
  ...
  parser.add_argument("--few_shot", default=-1, type=int, help="num of few-shot exampes")
  ...
  if args.do_train:
    ...
    train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, split=args.train_split, language=args.predict_languages, lang2id=lang2id, evaluate=False, few_shot=args.few_shot)
    oracle_datasets = {}
    oracle_bz = min(args.few_shot, args.per_gpu_train_batch_size)
    for oracle_lang in args.predict_languages.split(","):
      oracle_data = load_and_cache_examples(args, args.task_name, tokenizer, split=args.train_split, language=oracle_lang, lang2id=lang2id, evaluate=False, few_shot=args.few_shot)
      if len(oracle_data) != 0:
        oracle_sampler = RandomSampler(oracle_data)
        oracle_dataloader = DataLoader(oracle_data, sampler=oracle_sampler, batch_size=oracle_bz)
        oracle_datasets[oracle_lang] = list(oracle_dataloader)
      # elif oracle_lang == "en":
      #   logger.info("Ignore English dev data!")

    global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset, model, tokenizer, oracle_datasets, lang2id)
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    tokenizer = tokenizer_class.from_pretrained(args.output_dir+"checkpoint-best", do_lower_case=args.do_lower_case)
