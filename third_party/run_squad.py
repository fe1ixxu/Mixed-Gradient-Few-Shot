...
def SGS_backward(args, loss, model, optimizer, oracle_datasets):
  ...
  oracle_langs = list(oracle_datasets.keys())
  ...
  def get_random_batch():
    oracle_lang = random.choice(oracle_langs)
    ...
    inputs = {
      "input_ids": oracle_batch[0],
      "attention_mask": oracle_batch[1],
      "token_type_ids": None if args.model_type in ["xlm", "xlm-roberta", "distilbert"] else oracle_batch[2],
      "start_positions": oracle_batch[3],
      "end_positions": oracle_batch[4],
    }

    if args.model_type in ["xlnet", "xlm"]:
      inputs.update({"cls_index": oracle_batch[5], "p_mask": oracle_batch[6]})
      if args.version_2_with_negative:
        inputs.update({"is_impossible": oracle_batch[7]})
    if args.model_type == "xlm":
      inputs["langs"] = oracle_batch[7]
...
def train(args, train_dataset, model, tokenizer, oracle_datasets):
  """ Train the model """
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if random_num < 0.9:
        ...
        SGS_backward(args, loss, model, optimizer, oracle_datasets)
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False,
              language='en', lang2id=None, few_shot=-1):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
    ...
    files_to_load = args.train_file if not evaluate else args.predict_file
    files_to_load = files_to_load.split(",")[:-1] if not evaluate else files_to_load.split(",")
    logger.info("Loading files are {}".format(files_to_load))
    all_features = []
    for file_to_load in files_to_load:
      language = file_to_load.split(".")[-3]
      logger.info("Loading file {} with language {}".format(file_to_load, language))
      ...
      if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
        ...
        if evaluate:
          examples = processor.get_dev_examples(args.data_dir, filename=file_to_load, language=language)
        elif not evaluate:
          examples = processor.get_train_examples(args.data_dir, filename=file_to_load, language=language)
        ...
        if few_shot > 0 and language != "en":
          ...
          logger.info("few-shot samples are {}".format(str([f.input_ids for f in features])))
...
def load_and_cache_examples_oracle(args, tokenizer, file_to_load, evaluate=False, output_examples=False,
              language='en', lang2id=None, few_shot=-1):
  if args.local_rank not in [-1, 0] and not evaluate:
    # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    torch.distributed.barrier()
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
    ...
    logger.info("Loading oracle file {} with language {}".format(file_to_load, language))
    logger.info("Creating features from dataset file at %s", input_dir)
    ...
    if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
      ...
      if evaluate:
        ...
        examples = processor.get_train_examples(args.data_dir, filename=file_to_load, language=language)
      # elif not evaluate and few_shot > 0:
      #   examples = processor.get_dev_examples(args.data_dir, filename=args.train_file, language=language)
      ...
      oracle_sampler = RandomSampler(dataset)
      oracle_bz = args.few_shot #min(args.few_shot, args.per_gpu_train_batch_size)
      oracle_dataloader = DataLoader(dataset, sampler=oracle_sampler, batch_size=oracle_bz)

  if args.local_rank == 0 and not evaluate:
    # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    torch.distributed.barrier()
  return list(oracle_dataloader)
...
def main():
  ...
  parser.add_argument("--few_shot", type=int, default=-1, help="few_shot number")
  parser.add_argument("--init_checkpoint", action="store_true", help="init model")
  ...
  # Training
  if args.do_train:
    train_dataset = load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False, language=args.train_lang, lang2id=lang2id, few_shot=args.few_shot)
    oracle_datasets = {}
    files_to_load = args.train_file.split(",")[:-1]
    for file_to_load in files_to_load:
      lang = file_to_load.split(".")[-3]
      logger.info("Loading oracle {} dataset".format(lang))
      oracle_datasets[lang] = load_and_cache_examples_oracle(args, tokenizer, file_to_load, evaluate=False, output_examples=False, language=lang, lang2id=lang2id, few_shot=args.few_shot)

    global_step, tr_loss = train(args, train_dataset, model, tokenizer, oracle_datasets)
  ...
  with open(args.output_dir+"test_results.txt", "a") as f:
    f.writelines(["f1 ", args.eval_lang, " ", str(results["f1"]), "\n"])
  # return results
