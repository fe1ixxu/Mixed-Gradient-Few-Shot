...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, oracle_datasets, lang2id=None):
  """Train the model."""
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    for lg in langs:
      ...
      if few_shot > 0 and lg != "en":
        ...
        features_lg = features_lg[: few_shot]
        logger.info('Using few-shot learning on {} examples'.format(len(features_lg)))
        logger.info("few-shot samples are {}".format(str([f.input_ids for f in features_lg])))
...
def main():
  ...
  # Training
  if args.do_train:
    train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang=args.predict_langs, lang2id=lang2id, few_shot=args.few_shot)
    oracle_datasets = {}
    oracle_bz = min(args.few_shot * args.n_gpu if args.few_shot > 0 else args.per_gpu_train_batch_size * max(1, args.n_gpu), args.per_gpu_train_batch_size * max(1, args.n_gpu))
    for oracle_lang in args.predict_langs.split(","):
      oracle_data = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang=oracle_lang, lang2id=lang2id, few_shot=args.few_shot)
      if len(oracle_data) != 0:
        ...
        if oracle_lang == "en":
          ...
          oracle_dataloader = DataLoader(oracle_data, sampler=oracle_sampler, batch_size=len(oracle_data))
        oracle_datasets[oracle_lang] = list(oracle_dataloader)

    global_step, tr_loss = train(args, train_dataset, model, tokenizer, labels, pad_token_label_id, oracle_datasets, lang2id)
